...
import { Calendar, UserCheck, UserX, AlertTriangle, Clock, ArrowLeft } from 'lucide-react';
...
const AdminEmployeeAttendance = () => {
  ...
  const fetchHistory = async () => {
    try {
      ...
      const parsed: ParsedRecord[] = res.data
        .filter((rec: any) => {
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
        <div className="flex items-start gap-3">
          <button
            ...
            className="mt-0.5 flex-shrink-0 text-gray-500 hover:text-gray-900 bg-gray-100 hover:bg-gray-200 p-2 rounded-full transition-colors"
          >
          ...
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-sm text-gray-500 truncate">Viewing records for Employee ID: <span className="font-mono font-medium">{id}</span></p>
          </div>
        </div>
